...
class Scheduler<T>{
    ...
    public boolean remove (Timestamp<T> t){
        return objects.removeIf(i->i.equals(t));
    }
    public Timestamp <T> next(){
        return objects.stream().filter(i->i.getTime().isAfter(LocalDateTime.now())).min(Timestamp::compareTo).orElse(null);
    }
    public Timestamp <T> last(){
        return objects.stream().filter(i->i.getTime().isBefore(LocalDateTime.now())).max(Timestamp::compareTo).orElse(null);
    }
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return  objects.stream()
                .filter(i->i.getTime().isAfter(begin)&&i.getTime().isBefore(end)).collect(Collectors.toList());

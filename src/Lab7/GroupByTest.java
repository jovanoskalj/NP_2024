    ...
    static class Faculty {
        ...
        public Map<String, Double> studentsAverageGrade() {
            return records.stream().filter(
                    record -> record.getGrade() > 5
            ).sorted(Comparator.comparing(Record::getStudentId)).collect(Collectors.groupingBy(
                    Record::getStudentId, Collectors.averagingInt(Record::getGrade)
            ));
        }
        ...
        public Map<String, Long> coursesPassedStudentsCount() {
            return records.stream()
                    ...
                    .collect(Collectors.groupingBy(
                            Record::getCourseName,
                            TreeMap::new,
                            Collectors.mapping(Record::getStudentId, Collectors.toSet())
                    ))
                    .entrySet().stream()
